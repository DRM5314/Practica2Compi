...
import ejecutorAnalizador from '../jison/analizador';
import Tabla from '../manejadorAnalizador/Tabla';
import ManejadorTabla from '../manejadorTabla/ManejadorTabla';
...
const editor = (req: Request, res: Response) => {
    /*
    var tt = [];
...
const analizadores = (req: Request, res: Response) => {
    res.render('analizadores', {
        'titulo': 'Analizadores'
    });
};
const analizar = (req: Request, res: Response) => {
    ...
    //console.log(entradaTexto);    
    let arregloErrores: any[];
    try {

        var tabla: Tabla;
        let solicitudes = ejecutorAnalizador.parse(entradaTexto);
        solicitudes.mostrarInformacion();
        tabla = solicitudes.getTabla;
        let manejadorTabla: ManejadorTabla = new ManejadorTabla(tabla);
...
const regreso = (req:Request,res:Response)=>{
...
const editorUsuario = (req:Request,res:Response)=>{
    var texto = req.body.textoArea;
    
}
export default { editor, analizadores, analizar, regreso }